...
const FaceCanvas = dynamic(() => import("@/components/FaceCanvas"), {
  ssr: false,
  loading: () => (
    <div className="grid h-64 place-items-center text-zinc-500 text-sm">
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 flex flex-col">
      ...
      <main className="flex-1 lg:grid lg:grid-cols-2 lg:gap-5 lg:p-5 lg:max-w-7xl lg:mx-auto lg:w-full">
        ...
        <section className={`${view === "camera" ? "flex" : "hidden"} lg:flex flex-col gap-3 p-4 lg:rounded-3xl lg:bg-zinc-900/40 lg:ring-1 lg:ring-white/[0.08]`}>
          ...
          <FaceCanvas
            ...
            onAnalyze={(a) => {
              setAnalysis(a);
              setView("studio"); // auto-switch to studio after analysis on mobile
            }}
          />
          {selected && (
            <div className="flex items-center gap-2 rounded-xl bg-amber-400/10 px-3 py-2 ring-1 ring-amber-400/20 text-xs text-amber-300">
              ...
              AR: <span className="font-semibold ml-1 truncate">{selected.name}</span>
              <button onClick={() => setSelected(null)} className="ml-auto text-zinc-500 hover:text-zinc-300 flex-shrink-0">✕</button>
            </div>
          )}
          {/* Mobile hint */}
        ...
        <section className={`${view === "studio" ? "flex" : "hidden"} lg:flex flex-col gap-3 p-4 lg:rounded-3xl lg:bg-zinc-900/40 lg:ring-1 lg:ring-white/[0.08] lg:max-h-[90vh] lg:overflow-hidden`}>
          <div className="hidden lg:flex items-center justify-between flex-shrink-0">
            ...
            <span className="text-[10px] text-zinc-600 font-mono">claude-sonnet-4-6 · AI powered</span>
          </div>
          {/* Mobile back button */}
          ...
          <div className="flex-1 overflow-y-auto">
            <ProductPanel
              ...
              onSelect={(p) => {
                setSelected(p);
                setView("camera"); // switch to camera to see AR overlay
              }}
            />
          </div>
        </section>
      ...
      {/* ── Footer ── */}
      <footer className="border-t border-white/[0.05] px-4 py-2 text-[9px] text-zinc-700 flex justify-between">
